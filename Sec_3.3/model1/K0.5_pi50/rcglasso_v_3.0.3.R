...
# fitmean = flag used to fit the expected value
# scale = flag for scaling the matrix X
# nrho = number of rho-values
# maxrho = largest rho-value
# rhoratio = smallest rho-value is defined as rhoratio x maxrho
# nstp = number of iteration steps
# eps = argument used for convergence
# tol = tollerance value for the conditial expencted values
# verbose = flag for printing out information as iterations proceed.

rcglasso <- function(X, k, fitmean = FALSE, scale = TRUE, nrho = 50, maxrho, rhoratio, nstp = 500, eps = 1e-6, tol = 1e-5, verbose = FALSE){
