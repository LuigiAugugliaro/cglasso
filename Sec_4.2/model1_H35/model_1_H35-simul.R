...
k <- 3              # parameter used to control the parsity in Theta
prob_link <- k/p    # probability that a link will be present
H <- 35             # maximum number of variable that may be censored
...
for(j in 1:nsim){
    ...
    if(j %in% (10 * (1:10))) save.image("model_1_H35_tmp.RData")
}

save.image("model_1_H35.RData")
