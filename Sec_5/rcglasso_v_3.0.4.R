...
rcglasso <- function(X, k, fitmean = FALSE, scale = TRUE, nrho = 50, maxrho, minrho = 1e-3, nstp = 500, eps = 1e-6, tol = 1e-5, verbose = FALSE){
    ...
    id <- which(xv <= zero)
    if(length(id) > 0){
        msg <- paste("column", sQuote(id), "is removed because the standard deviation is zero", collapse = "\n")
        warning(msg)
        X <- X[, -id]
        R <- R[, -id]
        k <- k[-id]
    ...
    for(h in 1:nmodels){
        ...
        for(i in 1:nstp){
            ...
            if(fitmean) {
                ...
                if(verbose) cat("\n\nmax(dmuh, dthetah) = ", max(dmuh, dthetah), "\n\n")
        ...
        if(is.element(out$conv, 1:2)) {
            if(nmodels == 1) stop("rcglasso does not converge at rho = ", rho)
