...
# data: 05-02-2018
#
# Description: internal function used in rcglasso
...
rcglasso.fitTheta <- function(R, nobs, X, k, n_mis, row_mis, col_mis, rho, muh, thetah, nstp, tol, eps, verbose){
    ...
    thetah_old <- thetah
    conv <- 0
    if(verbose) cat(c("\t\ti", "dthetah", "dpar", "\n"))
    for(i in 1:nstp){
        T2_bar <- T2_obs
        for(h in 1:n_mis){
            ri <- col_mis[[h]]
        ...
        S_bar <- T2_bar / nobs
        out <- glasso(S_bar, rho = rho, penalize.diagonal = TRUE, wi.init = thetah_old)
        thetah_new <- (out$wi + t(out$wi)) / 2
        dthetah <- max(abs(thetah_new - thetah_old))
        if(verbose) cat(c("\t\t", i, dthetah, sum(diag(thetah_new)), sum(diag(S_bar))), "\n")
        if(dthetah < eps) {
            thetah_new <- thetah_old
            break
        }
